...
def siamese_model(input1_shape, input2_shape,
    ...
    if num_siam_layers > 0:
        siamese_layer = Dense(num_siam_nodes, activation = 'relu', name = "siamese_0")

        hidden_a = siamese_layer(input_a)
        hidden_b = siamese_layer(input_b)

    for i in range(1, num_siam_layers):
        siamese_layer = Dense(num_siam_nodes, activation = 'relu', name = "siamese_" + str(i))
    ...
    for i in range(num_hid_layers):
        merged_layer = Dense(num_hid_nodes, activation = 'relu', name = "hidden_" + str(i))(merged_layer)
...
def simple_model(input_shape, output_nodes, num_hid_layers, num_hid_nodes, dropout):
    input_layer = Input(shape = input_shape, name = "input")
    
    if num_hid_layers > 0:
        hidden_layer = Dense(num_hid_nodes, activation = 'relu', name = "hidden_0")(input_layer)
        hidden_layer = Dropout(dropout, name = "dropout_0")(hidden_layer)
    
    for i in range(1,num_hid_layers):
        hidden_layer = Dense(num_hid_nodes, activation = 'relu', name = "hidden_" + str(i))(hidden_layer)
